...
client.request = function (options) {
  ...
  this.runHandlers(this.opt, null, null, function () {
    ...
    that.ticlient.onerror = that.errorcb();
    that.ticlient.onload = that.successcb();
...
client.errorcb = function () {
  ...
  return function (err) {
    ...
    that.runHandlers(that.opt, res, err, function () {
      that.send(err, null);
    });
  };
};
...
client.successcb = function () {
  ...
  return function () {
    ...
    that.runHandlers(that.opt, res, null, function () {
      that.send(null, res);
    });
  };
};
